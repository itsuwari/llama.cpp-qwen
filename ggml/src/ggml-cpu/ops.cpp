...
static void ggml_compute_forward_get_rows_q(
    ...
    for (int64_t i = ir0; i < ir1; ++i) {
        ...
        if (i01 < 0 || i01 >= ne01) {
            GGML_ABORT("GET_ROWS bounds: dst=%s weights=%s ids=%s index=%lld rows=%lld row=%lld type=%s",
                    dst->name, src0->name, src1->name, (long long)i01,
                    (long long)ne01, (long long)i, ggml_type_name(type));
        }
...
void ggml_compute_forward_flash_attn_ext_indexed(
    ...
    GGML_ASSERT(v->type == GGML_TYPE_F16 || v->type == GGML_TYPE_Q8_0);
    ...
    const auto to_float_k = ggml_get_type_traits(k->type)->to_float;
    const auto to_float_v = ggml_get_type_traits(v->type)->to_float;
    GGML_ASSERT(to_float_k && to_float_v);
    std::vector<float> key_row(dk);
    std::vector<float> value_row(dv);
    ...
    for (int64_t ir = params->ith; ir < nr; ir += params->nth) {
        ...
        for (int64_t ii = 0; ii < indices->ne[0]; ++ii) {
            ...
            const char * pk = (const char *) k->data + (int64_t) idx*k->nb[1] + ik2*k->nb[2] + iq3*k->nb[3];
            const char * pv = (const char *) v->data + (int64_t) idx*v->nb[1] + ik2*v->nb[2] + iq3*v->nb[3];
            to_float_k(pk, key_row.data(), dk);
            to_float_v(pv, value_row.data(), dv);

            float score = 0.0f;
            for (int64_t id = 0; id < dk; ++id) {
                score += pq[id]*key_row[id];
            }
            score *= scale;
            if (logit_softcap != 0.0f) {
            ...
            for (int64_t id = 0; id < dv; ++id) {
                out[id] = out[id]*alpha + value_row[id]*beta;
            }
            s = s*alpha + beta;
            m = m_new;
        }

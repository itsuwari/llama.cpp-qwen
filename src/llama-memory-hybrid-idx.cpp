...
bool llama_memory_hybrid_idx::seq_rm(llama_seq_id seq_id, llama_pos p0, llama_pos p1) {
    ...
    if (res) {
        qsa_pending.clear();
        static const bool preserve=std::getenv("LLAMA_QSA_PRESERVE_PREFIX")!=nullptr;
        if (preserve && mem_idx && mem_idx->get_n_stream()==1 && seq_id==0 && p0>=0 && p1<0) {
            // Only retain a complete prefix whose every source cell remains live,
            // belongs to this sequence, and precedes the rollback position.
            // Later blocks are re-evaluated by the existing descriptor comparison.
            const auto & cells=mem_idx->get_cells(seq_id);
            uint64_t kept=0,dropped=0;
            for (auto & entry : qsa_blocks) {
                if(entry.second.size()!=1) {entry.second.clear();continue;}
                auto & blocks=entry.second[0];
                size_t keep=0;
                for (const auto & block : blocks) {
                    // Preserve only ordinary contiguous text blocks owned solely
                    // by this sequence. Shared/ranked layouts take the full-clear path.
                    const uint32_t ratio=entry.first;
                    bool valid=block.cache_stream==0 && block.cells.size()==ratio && block.start>=0 &&
                        int64_t(block.start)+ratio<=p0 &&
                        std::find(block.cells.begin(),block.cells.end(),block.key_cell)!=block.cells.end();
                    for (auto position:block.pos) valid &= position==block.start;
                    for (uint32_t ir=0; valid && ir<ratio; ++ir) {
                        const uint32_t cell=block.cells[ir];
                        if(cell>=mem_idx->get_size() || cells.is_empty(cell) || cells.seq_count(cell)!=1 ||
                           !cells.seq_has(cell,seq_id) || cells.pos_get(cell)!=block.start+int32_t(ir)) valid=false;
                    }
                    if(!valid)break;
                    ++keep;
                }
                kept+=keep;dropped+=blocks.size()-keep;blocks.resize(keep);
            }
            static int traced=0;
            if(std::getenv("LLAMA_R95_REUSE_TRACE") && traced++<24)
                fprintf(stderr,"R95_QSA_PREFIX p0=%d kept=%llu dropped=%llu\n",int(p0),(unsigned long long)kept,(unsigned long long)dropped);
        } else {
            qsa_blocks.clear();
        }
    }
...
void llama_memory_hybrid_idx::state_read(llama_io_read_i & io, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    try {
        ...
        qsa_pending.clear();
        if(std::getenv("LLAMA_R95_REUSE_TRACE")) fprintf(stderr,"R95_QSA_STATE_CLEAR flags=%u\n",unsigned(flags));
        qsa_blocks.clear();
...
uint32_t llama_memory_hybrid_idx::get_qsa_update_capacity(
    ...
    // Stabilize the small-decode update shape. Extra rows recompute distinct
    // already-existing block keys from their original cells and positions.
    auto padded_capacity = [&](const qsa_layout & layout) -> uint32_t {
        const uint32_t actual=std::max<uint32_t>(1,layout.updates.size());
        static int traced=0;
        if(std::getenv("LLAMA_R95_REUSE_TRACE") && ubatch.n_tokens<=16 && traced++<24)
            fprintf(stderr,"R95_QSA_CAP actual=%u blocks=%zu nt=%u n_kv=%u\n",actual,layout.streams[0].blocks.size(),ubatch.n_tokens,n_kv);

        static const int pad=std::getenv("LLAMA_QSA_UPDATE_PAD")?std::atoi(std::getenv("LLAMA_QSA_UPDATE_PAD")):0;
        if((pad==8 || pad==16) && ubatch.n_tokens<=16 && n_stream==1 && actual<=uint32_t(pad) &&
           layout.streams[0].blocks.size()>=size_t(pad)) return uint32_t(pad);
        return actual;
    };
...
void llama_memory_hybrid_idx::set_input_qsa_cached(
    ...
    if (std::getenv("LLAMA_QSA_AUDIT_KEYS") && n_stream==1 && n_tokens<=16) {
        ...
        GGML_ASSERT(!stream.ranked && stream.seq_id==0 && stream.seq_pos_min==0);
        const auto & audit_cells=mem_idx->get_cells(0);
        for (size_t b=0; b<stream.blocks.size(); ++b) {
            const auto & block=stream.blocks[b];
            GGML_ASSERT(block.start==int32_t(b*ratio));
            for (uint32_t j=0;j<ratio;++j) GGML_ASSERT(audit_cells.pos_get(block.cells[j])==int32_t(b*ratio+j));
            for (auto pos:block.pos) GGML_ASSERT(pos==int32_t(b*ratio));
        }
    }
    ...
    if (std::getenv("LLAMA_QSA_UPDATE_PAD") && n_stream==1) {
        for (const auto & block : layout.streams[0].blocks) {
            if (n_update==n_updates) break;
            const int64_t dest=int64_t(block.cache_stream)*cache_size+block.key_cell;
            if (std::find(update_idx_data,update_idx_data+n_update,dest)==update_idx_data+n_update)
                append_update(0,block);
        }
        // Distinct destinations avoid overlapping writes in SET_ROWS.
        GGML_ASSERT(n_update==n_updates);
    }
